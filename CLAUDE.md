...
- 쓰기, 변경 작업이 일어나면 해당 사항에 대해 반드시 테스트 코드를 작성한다.
  - 테스트 코드도 역시나 현재 구현된 코드와 구조를 동일하게 따라야 한다.
  - **테스트 코드 작성 시, 인터페이스(Port)가 있다면 인터페이스(Port) 기준으로 테스트를 작성한다. 절대 구현체를 기준으로 작성하지 않는다.**
